...
object OdgovorRepository {
   ...
   suspend fun getOdgovoriAnketa(idAnketeTaken:Int):List<Odgovor>?{
        //vraća listu odgovora za anketu,
        //praznu listu ako student nije odgovarao ili nije upisan na zadanu anketu
       return withContext(Dispatchers.IO) {
           var response = ApiAdapter.retrofit.getOdgovori(AccountRepository.getHash(),idAnketeTaken)
    ...
    private fun zaokruziProgres(progres:Float):Int{
        ...
        if(progres>0F && progres<0.2){
            ...
            if(0.4F+0.1<progres) rez=0.6F else rez=0.4F
        }else if(progres>0.6F && progres<0.8F){
            if(0.6F+0.1F<progres) rez=0.8F else rez=0.6F
        }else if(progres>80 && progres<1F){
            if(0.8F+0.1F<progres) rez=1F else rez=0.8F
        }
